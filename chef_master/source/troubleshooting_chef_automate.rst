...
By default, Chef Automate build nodes/runners generated by ``automate-ctl install-build-node`` or ``automate-ctl install-runner``, respectively are tagged
as ``delivery-build-node`` and ``delivery-job-runner``. If your delivery.rb contains a custom search query
(``delivery['default_search']`` is set), try appending ``" OR tags:delivery-build-node"`` ``"OR tags:delivery-job-runner"``to your query.

The short story about build-node and runner configuration is this at minimum.

If your Automate system has Builders(Push Jobs), then your projects should have this config item in .delivery/config.json
...
If your Automate system has Runners, then your projects should have this config item in .delivery/config.json
...
For further detail: If you are trying to debug a particular build node or runner and want to ensure that one is available for your projects,
