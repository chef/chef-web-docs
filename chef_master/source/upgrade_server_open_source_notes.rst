...
* An organization must be created. Add the organization name as the value for the ``default_orgname`` setting in the ``/etc/opscode/chef-server.rb`` file. This will ensure that API requests made to the server prior to the upgrade process can still be made after.
