...
Because the contents of encrypted data bag items are not visible to
the chef-server, searches against data bags with encrypted items will
not return items whose unencrypted content would match the search.
.. end_tag
