...
Chef Automate's GitHub integration is designed for use with both GitHub.com and GitHub
...
#. A Personal Access token (OAuth token) generated by your GitHub service account. To create a Personal Access token, sign in to GitHub as your service account.
...
This procedure is only needed if you are connecting to GitHub Enterprise **and** your GitHub Enterprise server uses a self-signed SSL certificate.

.. note:: Even when trusted, self-signed certificates only work if the certificate subject is the same as the host running the service. For example, if the certificate subject is an IP address such as ``10.10.10.10`` and GitHub Enterprise server is reachable at `github.example.com`, then the URL ``https://github.example.com`` will fail SSL certificate validation while the URL ``https://10.10.10.10`` will pass SSL certificate validation.
