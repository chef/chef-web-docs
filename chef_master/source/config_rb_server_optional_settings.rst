...
``topology``
   The topology of the Chef Infra Server. Possible values: ``ha``, ``manual``, ``standalone``, and ``tier``. Default value: ``'standalone'``.
...
The **opscode-authz** service is used to handle authorization requests to the Chef Infra Server.
