...
The example above is using audit cookbook version 2.0. Further information is available at `Reporting to Chef Compliance via Chef Server <https://github.com/chef-cookbooks/audit#reporting-to-chef-compliance-via-chef-server.html>`____. For more configuration examples supported ``audit`` cookbook 4.0.0 and later, see `setting up visibility on Chef automate </data_collection.html>`__ and for supported audit cookbook configurations, see `Audit Cookbook </audit_cookbook.html>`__. For more information and examples on how to use the audit cookbook, see the `audit project repo in GitHub <https://github.com/chef-cookbooks/audit.html>`___.
