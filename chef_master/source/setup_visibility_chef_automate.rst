...
|chef server| can be configured to send messages to |automate| whenever an action is taken on a |chef server| object, such as when a cookbook is uploaded to 
...
.. warning:: Failure to set the ``fqdn`` field to the same value will result in |automate| treating data from each of these front-end servers as separate Chef servers.
