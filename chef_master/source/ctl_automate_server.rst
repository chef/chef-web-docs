...
.. note:: ``knife bootstrap`` is used underneath so custom configurations can be installed on the runner by using the ``client.d`` copying feature. All config files inside ``client.d`` directory that end in ``.rb`` are loaded and get copied into ``/etc/chef/client.rb``; other non-``.rb`` files are ignored.
