...
* **Amazon is now platform_family of amazon** As time has gone on Amazon Linux has become less and less like other RHEL derivatives. We're now detecting amazon as its own platform_family to make writing Amazon Linux compatible cookbooks easier
...
* **Removal of SBT Detection** The latest versions of sbt no longer include a sbt --version command and other methods of version detection require setting up a project in the working directory. Until a better version detection method can be determine we're removed sbt detection.
