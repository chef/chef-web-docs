...
.. note:: This topic describes an install procedure for a legacy product, Chef Delivery. The features in Chef Delivery are part of the workflow capabilities in |automate|. See :doc:`Upgrade Chef Automate </upgrade_chef_automate>` to learn how to upgrade to |automate| from the ``delivery-cluster`` setup used by Chef Delivery.
