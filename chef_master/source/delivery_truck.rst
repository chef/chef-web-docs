...
**Project Cookbook**
   A project cookbook is a cookbook that is used to deploy your project's software onto an infrastructure node. You can have one or many project cookbooks to control deploying your project's software. Every project cookbook's default recipe will be executed by Chef Client on your infrastructure nodes to deploy your software. Delivery Truck's provision recipe will parse your project's root for a ``metadata.rb`` and ``metadata.json`` file as well as directories of the structure ``cookbooks/<project_cookbooks>`` that also contain a ``metadata.rb`` and ``metadata.json`` file to detect your project cookbooks.
