...
* The workstation is the location from which all of |chef| is managed, including :doc:`installing the Chef DK </install_dk>`, :doc:`authoring cookbooks </cookbooks>`, and using tools like :doc:`Kitchen </kitchen>`, `chef-zero (a command-line tool that runs locally as if it were connected to a real Chef server) <https://docs.chef.io/ctl_chef_client.html#run-in-local-mode>`__, command-line tools like :doc:`Knife (for interacting with the Chef server) </xxxxx>` and :doc:`chef (for interacting with your local chef-repo) </xxxxx>`, and resources like :doc:`core Chef resources (for building recipes) </xxxxx>` and :doc:`InSpec (for building security and compliance checks into your workflow) </xxxxx>`.
