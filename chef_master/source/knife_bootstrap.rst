...
As of Chef 12.8 you can create a ``.chef/client.d`` directory on your workstation and the contents of that ``client.d`` directory will be copied to the system being bootstrapped by the ``knife bootstrap`` command. You can also set the ``client_d_dir`` option in ``knife.rb`` to point to an arbitrary directory instead of ``.chef/client.d`` and the contents of that directory will be copied to the system being bootstrapped. All config files inside ``client.d`` directory that end in ``.rb`` are loaded and get copied into ``/etc/chef/client.rb``; other non-``.rb`` files are ignored.
