...
The type of text editor that is used by knife can be configured by adding an entry to your knife.rb configuration file or by setting an ``EDITOR`` environment variable. For example, to configure the text editor to always open with vim, add the following to your knife.rb file:
