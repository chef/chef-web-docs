...
.. important:: All members of the backend cluster MUST contain identical sets of the ``postgresql.md5_auth_cidr_addresses`` subnet rules set for all systems, frontend and backend. This guarantees that when a cluster change of state occurs, all members can still communicate with one another as needed. Any other configuration is at risk of having unsuccessful failovers where followers are unable to communicate with the new leader, or the frontend cluster is unable to communicate with the new leader. To be clear, the ``postgresql.md5_auth_cidr_addresses`` setting is only applied on the backends. Not on the frontends. But the value set on the backends should contain the subnets of the frontends if the subnets are different from that of the backends' subnets. And all backends should have an identical setting for this attribute.
