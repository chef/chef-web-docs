...
* While Apache Solr is used in standalone Chef server instances,
  it is replaced with Elasticsearch in the highly available Chef server cluster. Elasticsearch provides more flexible clustering options while maintaining search API compatibility with Apache Solr.
...
* Writes to the search engine and the database are handled
  ...
  cluster writes to both the search engine and the database
...
* A load balancer between the rest of the network, and the frontend
  ...
  stored on each node individually in the frontend group, the load
...
Before creating the backend HA cluster and building at least one Chef server to be part of the frontend group, verify that:
...
A backend cluster is expected to run in a trusted environment. This means that untrusted users communicating with and/or eavesdropping on services provided by the backend cluster can potentially view sensitive data.
...
The following services run on each node in the backend cluster. The user account under which the service runs is listed the second column:
...
Common 'Runit' flags for backend services
-----------------------------------------------------
