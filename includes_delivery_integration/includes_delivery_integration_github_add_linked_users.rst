...
#. From a local checkout of a |delivery| project, run the appropriate |delivery| command that associates a |github| user with a |delivery| user.

   .. note:: The |delivery_cli| commands are for a user to link their own account to |github|, or others if the user has the **Admin** role; ``api`` is an argument to the |delivery_cli| command. The ``delivery-ctl`` command can only be run by an administrator from the |delivery| server and can affect any user.
