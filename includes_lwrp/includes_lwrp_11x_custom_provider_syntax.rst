...
This section shows some of the common structural elements that appear in a custom provider that is built in a way that leverages platform resources (such as |resource file|, |resource template|, or |resource package|). Remember:
...
* ``whyrun_supported?`` indicates whether a custom provider can be run in |whyrun| mode
