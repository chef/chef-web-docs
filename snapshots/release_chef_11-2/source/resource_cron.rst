...
``user``
   **Ruby Type:** String
   ...
   The name of the user that runs the command. If the ``user`` property is changed, the original ``user`` for the crontab program continues to run until that crontab program is deleted. Default value: ``root``.
...
.. tag resource_cron_providers

This resource has the following providers:

``Chef::Provider::Cron``, ``cron``
