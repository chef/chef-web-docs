...
: Use to override the `config.vm.communicator` setting in Vagrant. For example, when a base box is a Windows operating system that doesn't have SSH installed and enabled, Vagrant won't be able to boot without a custom Vagrant file.
...
: The internal hostname for the instance. This isn't required when connecting to a Vagrant virtual machine. Set this to `false` to prevent this value from being rendered in the default Vagrantfile.
