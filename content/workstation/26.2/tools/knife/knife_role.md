...
[menu]
  [menu.workstation_26_2]
    title = "knife role"
    identifier = "workstation_26_2/tools/knife/reference/knife_role.md knife role"
...
{{< readfile file="content/workstation/26.2/reusable/md/knife_role_summary.md" >}}

{{< note >}}

To add a role to a node and then build out the run-list for that node,
use the [knife node](/workstation/26.2/tools/knife/knife_node/) subcommand and its `run_list add`
argument.
...
Use the `create` argument to add a role to Chef Infra Server. Role
data is saved as JSON on Chef Infra Server.
...
: The description of the role. This value populates the description
    field for the role on Chef Infra Server.
...
Use the `delete` argument to delete a role from Chef Infra Server.
...
Use the `edit` argument to edit role details on Chef Infra Server.
...
To view a list of roles on Chef Infra Server and display the URI for
