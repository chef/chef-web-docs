...
[menu]
  [menu.workstation_26_2]
    title = "knife opc"
    identifier = "workstation_26_2/tools/knife/reference/plugin_knife_opc.md knife opc"
...
If your `chef_server_url` configuration ends with `/organizations/ORG_NAME` (as shown above), this setting will default to `https://chef.example.com/`.

{{< note >}}

On Chef Infra Server, the majority of the commands provided by this plugin can be accessed with `chef-server-ctl` wrapper commands. [See chef-server-ctl for details](/server/ctl_chef_server//).
