...
[menu]
  [menu.workstation_26_2]
    title = "Configure knife"
    identifier = "workstation_26_2/tools/knife/config_rb.md config.rb (knife.rb)"
...
The `config.rb` file is a replacement for the `knife.rb` file. The `config.rb` file has identical settings and behavior to the `knife.rb` file. Chef Infra Client looks first for the presence of the `config.rb` file and if it isn't found, then looks for the `knife.rb` file.
...
: The URL of Chef Infra Server. For example:
  ...
  {{< note >}}
  If changes need to be made to any global end points like user or user keys, use [`knife exec`]({{< relref "knife_exec" >}}) with the `--server-url` flag to set `chef_server_url` to `https://localhost/`.
  {{< /note >}}
...
`cookbook_copyright`
: The name of the copyright holder. This option places a copyright notice that contains the name of the copyright holder in each of the pre-created files. If this option isn't specified, a copyright name of "COMPANY_NAME" is used instead; it can  be modified later.
...
: The minimum required version of data bag encryption. Possible values: `1` or `2`. Set this to `2` unless you're running Chef Infra Client 11.5 or lower. For example:
...
`local_mode`
: Run Chef Infra Client in local mode. This allows all commands that work against Chef Infra Server to also work against the local chef-repo. For example:
...
: The name of the node. This may be a username with permission to authenticate to Chef Infra Server or it may be the name of the machine from which knife is run. For example:
...
: Set the verify mode for HTTPS requests.
  ...
  - Use `:verify_peer` to do validation of all SSL certificates, including Chef Infra Server connections, S3 connections, and any HTTPS **remote_file** resource URLs used in a Chef Infra Client run. This is the recommended setting.
...
: The name of the chef-validator key that's used by Chef Infra Client to access Chef Infra Server during the initial Chef Infra Client run when not using validatorless bootstrapping. For example:
...
`verify_api_cert`
: Verify the SSL certificate on Chef Infra Server. When `true`, Chef Infra Client always verifies the SSL certificate. When `false`. Chef Infra Client uses the value of `ssl_verify_mode` to determine if the SSL certificate requires verification. Default value: `false`.

`versioned_cookbooks`
: Append cookbook versions to cookbooks. Set to `false` to hide cookbook versions: `cookbooks/apache`. Set to `true` to show cookbook versions: `cookbooks/apache-1.0.0` and/or `cookbooks/apache-1.0.1`. When this setting is `true`, `knife download` downloads ALL cookbook versions, which can be useful if a full-fidelity backup of data on Chef Infra Server is required. For example:
...
In certain situations the proxy used by Chef Infra Server requires
...
to access Chef Infra Server: HTTP or HTTPS.

If Chef Infra Server is configured to use HTTP, add the following
settings:
...
`knife[:ssh_password]`
: The SSH password. This can be used to pass the password directly on the command line. If this option isn't specified (and a password is required) knife prompts for the password.
...
Review the full list of [optional settings](/workstation/26.2/tools/knife/config_rb_optional_settings/) that can be added to the `config.rb` file.
Many of these optional settings shouldn't be added to `config.rb`, and the reasons vary.

For example, using `--yes` as a default causes knife to always assume `Y` as the response to any prompt, which may lead to undesirable outcomes.
Other settings, such as `--hide-healthy` (used only with the `knife status` subcommand) or `--bare-directories` (used only with the `knife list` subcommand), probably aren't used often enough to justify adding them to `config.rb`.

In general, if an optional setting isn't listed on the main `config.rb` [page](/workstation/26.2/tools/knife/config_rb/), add it only after careful consideration.
Don't use optional settings in a production environment until you've validated the setting's performance in a safe testing environment.
