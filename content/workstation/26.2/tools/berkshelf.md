...
[menu]
  [menu.workstation_26_2]
    title = "Berkshelf"
    identifier = "workstation_26_2/tools/berkshelf.md Berkshelf"
    parent = "workstation_26_2/tools"
    weight = 10
+++
...
{{< warning >}}
[Policyfiles](/client/latest/policy/policyfile/) are the preferred pattern for creating immutable collections of cookbooks, cookbook dependencies, and attributes. Berkshelf is no longer under active development. Please replace Berkshelf with Policyfiles
...
[Policyfiles](/client/latest/policy/policyfile/) are the recommended pattern for including and resolving cookbooks. Policyfiles provide more predictability, since dependencies are only resolved once, and a much improved way of promoting cookbooks from dev to testing, and then to production.
...
The location and authentication details for Chef Infra Server will
...
If the `api_key` option isn't given, it will use the value of the
...
file](/client/latest/cookbooks/config_rb_metadata/#cookbook-version-constraints).
...
same system. It doesn't cache the target cookbook, ensuring that the
...
Groups can be defined with blocks:
...
The `gecode` solver matches the engine used by Chef Infra Server, so it reflects the behavior of Chef Infra Server in selecting cookbooks:
...
environment on Chef Infra Server.
...
: An array of cookbook groups that won't be listed.

`-o [GROUP, GROUP, ...]`, `--only [GROUP, GROUP, ...]`

: An array of cookbook groups to be listed. When this option is used, cookbooks that exist in groups not listed won't be listed.
...
: An array of cookbook groups that won't be checked for version constraints.

`-o [GROUP, GROUP, ...]`, `--only [GROUP, GROUP, ...]`

: An array of cookbook groups to be checked for version constraints. When this option is used, cookbooks that exist in groups not listed won't be checked for version constraints.
...
: An array of cookbook groups that won't be vendored, and then  archived.

`-o [GROUP, GROUP, ...]`, `--only [GROUP, GROUP, ...]`

: An array of cookbook groups to be vendored, and then archived. When this option is used, cookbooks that exist in groups not listed won't be vendored or archived.
...
: An array of cookbook groups that won't be updated.

`-o [GROUP, GROUP, ...]`, `--only [GROUP, GROUP, ...]`

: An array of cookbook groups to be updated. When this option is used, cookbooks that exist in groups not listed won't be updated.
...
: An array of cookbook groups that won't be uploaded.

`--force`

: Use to upload any named cookbook even if that cookbook exists on Chef Infra Server and is frozen.

`--halt-on-frozen`

: Use to exit the command with a non-zero exit code if this version of a cookbook already exists on Chef Infra Server.

`-o [GROUP, GROUP, ...]`, `--only [GROUP, GROUP, ...]`

: An array of cookbook groups to be uploaded. When this option is used, cookbooks that exist in groups not listed won't be uploaded.

`--no-freeze`

: A frozen cookbook requires changes to that cookbook to be submitted as a new version of that cookbook. Use this option to prevent this cookbook from being frozen. Default value: `false` (that's, "frozen").

`--ssl-verify`

: Use to enable (`true`) or disable (`false`) SSL verification when uploading cookbooks to Chef Infra Server.

`-s`, `--skip-syntax-check`

: Use to skip Ruby syntax checking when uploading a cookbook to Chef Infra Server. Default value: `false`.
...
: An array of cookbook groups that won't be vendored.

`-o [GROUP, GROUP, ...]`, `--only [GROUP, GROUP, ...]`

: An array of cookbook groups to be vendored. When this option is used, cookbooks that exist in groups not listed won't be vendored.
