...
[menu]
  [menu.workstation_26_2]
    title = "ChefSpec"
    identifier = "workstation_26_2/tools/chefspec.md ChefSpec"
...
- `expect()` defines the expectation: the sum of one plus one equals two---`expect(sum).to eq(2)`---and doesn't equal three--`expect(sum).not_to eq(3)`
...
describe 'include_recipe::default' do
  ...
  it 'doesn't include the `not` recipe' do
...
describe 'multiple_actions::sequential' do
  ...
  it 'doesn't match other actions' do
