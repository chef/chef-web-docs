...
[menu]
  [menu.workstation_26_1]
    title = "knife ssh"
    identifier = "workstation_26_1/tools/knife/reference/knife_ssh.md knife ssh"
    parent = "workstation_26_1/tools/knife/reference"
+++
<!-- markdownlint-disable-file MD036 -->

{{< readfile file="content/workstation/26.1/reusable/md/knife_ssh_summary.md" >}}
...
: The search query used to return a list of servers to be accessed using SSH and the specified `SSH_COMMAND`. This option uses the same syntax as the search subcommand. If the `SEARCH_QUERY` doesn't contain a colon character (`:`), then the default query pattern is `tags:*#{@query}* OR roles:*#{@query}* OR fqdn:*#{@query}* OR addresses:*#{@query}*`, which means the following two search queries are effectively the same:
...
`macterm`, `screen`, or `tmux`. If the node doesn't have the shell type
