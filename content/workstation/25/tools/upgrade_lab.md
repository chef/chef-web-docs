...
[menu]
  [menu.workstation_25]
    title = "Upgrade Lab"
    identifier = "workstation_25/chef_workstation/upgrade_lab.md Upgrade Lab"
...
This guide covers the straightforward pattern of upgrading from Chef Infra Client 12 to Chef Infra Client 16, including upgrading Chef Infra Server as you go. While we think this guide is the smoothest path forward, it isn't meant to exclude other approaches.
...
- One or more nodes
  ...
  - Minimum 512 MB RAM
  - [Recommended](/client/latest/install/system_requirements/#chef-infra-client) 5GB space
...
- Review the [platform and system requirements](/workstation/25/install/) for Chef Workstation
- Install or upgrade to the Chef Workstation [latest version](https://www.chef.io/downloads)

Chef doesn't prescribe any specific editor. However, the [Chef Infra extension](https://marketplace.visualstudio.com/items?itemName=chef-software.Chef) for [Visual Studio Code](https://code.visualstudio.com/) features several code generators and helpful features, such as running Cookstyle each time you save a recipe.

### Server Backup

We recommend performing a backup before starting any server upgrade process. Follow the [Chef Infra Server Backup](/server/server_backup_restore/) documentation before starting your Upgrade Lab.

### Infrastructure Requirements

#### Credentials

Your credentials are set up using [knife profiles](/workstation/25/tools/knife/set_up/#knife-profiles). This method allows you to keep your keys in a `credentials` file, and makes switching between credentials easier.
...
You must have a user key for both of Chef Infra Servers and you must be able to connect to both Chef Infra Servers from your development workstation.
...
If your Chef Infra Server uses data bags, you will need to download the `data_bags` directory into your chef repo with the `chef exec knife download data_bags` command. This command doesn't support embedded keys in credentials files. If you use embedded keys, move the key to a key file.
...
Next, upload the remaining data to the new server. Note that `knife upload` command doesn't support embedded keys in credentials files, so you must place the key in a key file.
