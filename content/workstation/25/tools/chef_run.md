...
[menu]
  [menu.workstation_25]
    title = "chef-run executable"
    identifier = "workstation_25/tools/chef_run.md chef-run executable"
...
WinRM connections only support password authentication. Provide username and password as shown in the example, or with the `--user` and `--password` flags.
...
You can specify all the Chef Infra [resources](/client/latest/resources/) in the command line. Enter the `chef-run` command first, followed by the resource type in the second place, and the resource name in the third place. For example:
...
If you run `chef-run host1 my_cookbook` and the current directory doesn't have a cookbook named `my_cookbook`, then `chef-run` searches the configured paths, with those configured in `~/.chef-workstation/config.toml` taking priority over those in `~/.chef/config.rb`.
...
When converging a target node `chef-run` creates a Policyfile bundle that includes the cookbook specified. If the cookbook you specified has its own [`Policyfile.rb`](/client/latest/policy/config_rb_policyfile/) that will be respected.
...
You can specify different cookbook sources in `Policyfile.rb`, including a private supermarket. See the [Policyfile documentation](/client/latest/policy/config_rb_policyfile/) for examples.
