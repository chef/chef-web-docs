...
[menu]
  [menu.workstation_26_0]
    ...
    identifier = "workstation_26_0/tools/knife/plugins/plugin_knife_custom.md Writing Custom Plugins"
    parent = "workstation_26_0/tools/knife/plugins"
    weight = 20
+++

<!-- markdownlint-disable-file MD033 MD037 -->

{{< readfile file="content/workstation/26.0/reusable/md/plugin_knife_summary.md" >}}
...
- `def run` is the Ruby code that's executed when the command is run

and where for each command-line option:

- `:short` defines the short option name
- `:long` defines the long option name
- `:description` defines a description that's displayed when a user
...
option :some_type_of_string_value,
  ...
  :description => "This isn't a random string value.",
  :default => 47
```

and can specify code that's run to determine the option's value:
...
**EXAMPLE COMMANDS**
knife example
  -s, --some-type-of-string-value     This isn't a random string value.
...
- `unless name_args.size == 1` is used to check the number of
    arguments given; the command should fail if the input doesn't make
    sense
...
execute search queries for objects on Chef Infra Server. For
