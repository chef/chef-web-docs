...
[menu]
  [menu.workstation_26_0]
    title = "knife winrm"
    identifier = "workstation_26_0/tools/knife/reference/knife_winrm.md knife winrm"
...
Review the list of [common options](/workstation/26.0/tools/knife/common_options/) available to this (and all) knife subcommands and plugins.
...
connections between the workstation and Chef Infra Server. For example:
...
When knife is executed from a Windows system, it's no longer necessary to make additional configuration of the WinRM listener on the target node to enable successful authentication from the workstation. It's sufficient to have a WinRM listener on the remote node configured to use the default configuration for `winrm quickconfig`. This is because `knife winrm` supports the Windows negotiate protocol, including NTLM authentication, which matches the authentication requirements for the default configuration of the WinRM listener.
...
{{< readfile file="content/workstation/26.0/reusable/md/knife_windows_winrm_ports.md" >}}
...
: The peer verification mode that's used during WinRM communication. Possible values: `verify_none` or `verify_peer`. Default value: `verify_peer`.
