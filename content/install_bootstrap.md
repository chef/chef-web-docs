...
The Chef Infra bootstrap process is validatorless by default. If you receive a warning during a bootstrap that a validator key is in use, remove the configuration for this legacy bootstrap mode. Edit your [config.rb (knife.rb)](/workstation/latest/tools/knife/config_rb/) file and remove any `validation_key` or `validation_client_name` entries.
