...
The `remote_file` resource has a new property `ssl_verify_mode` which allows you to control TLS/SSL validation at the property level. This can be used to verify certificates (Chef Infra Client's defaults) with `:verify_peer` or to skip verification in the case of a self-signed certificate with `:verify_none`. Thanks [@jaymzh](http://github.com/jaymzh).
...
| --winrm-ssl-verify-mode MODE | --winrm-no-verify-cert | Mode is not accepted. When flag is present, TLS/SSL cert will not be verified. Same as original mode of 'verify\_none'. [1] |
| --winrm-transport TRANSPORT | --winrm-ssl | Use this flag if the target host is accepts WinRM connections over TLS/SSL. [1] |
...
:   Verify the TLS/SSL certificate on the Chef server. When `true`, Chef
    Client always verifies the TLS/SSL certificate. When `false`, Chef
    Client uses the value of `ssl_verify_mode` to determine if the TLS/SSL
...
Chef server 12 enables TLS/SSL verification by default for all requests made
...
For example, without downloading the TLS/SSL certificate, the following
knife command:
...
See [TLS/SSL Certificates](/chef_client_security/#ssl-certificates) for
more information about how knife and Chef Client use TLS/SSL certificates
