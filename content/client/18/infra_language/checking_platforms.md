...
Use the `platform?` helper method to ensure that certain actions are run for specific platforms. The `platform?` method will return true if one of the listed parameters matches the `node['platform']` attribute that's detected by [Ohai](/client/18/features/ohai/) during every Chef Infra Client run.
...
Use the `platform_family?` method to ensure that certain actions are run for specific platform families. The `platform_family?` method will return true if one of the listed parameters matches the `node['platform_family']` attribute that are detected by [Ohai](/client/18/features/ohai/) during every Chef Infra Client run.
...
{{< readfile file="content/client/18/reusable/md/cookbooks_version_constraints_operators.md" >}}
