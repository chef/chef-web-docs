...
1. Define your recipe with the top-level `resources` key containing an array of items where each item has the following:
    ...
    - the [`package` resource]({{< relref "/client/19/resources/bundled/package/" >}}) uses the `install` action and the `version` property to install Nginx 1.18.0.
    - the [`service` resource]({{< relref "/client/19/resources/bundled/service/" >}}) uses the `enable` and `start` actions to enable and start Nginx.

## Examples

### Basic file management

Use the [`directory` resource]({{< relref "/client/19/resources/bundled/directory">}}) to create the `/opt/app_name` directory and apply owner and group permissions to the directory. Use the [`file` resource]({{< relref "/client/19/resources/bundled/">}}) to create the `/opt/app_name/config.txt` file, add text to the file, and apply file owner and group permissions to the file.
...
Use the [`package` resource]({{< relref "/client/19/resources/bundled/package">}}) to install Nginx and curl. Then use the [`service` resource]({{< relref "/client/19/resources/bundled/service">}}) to enable and start Nginx.
...
Use the [`group` resource]({{< relref "/client/19/resources/bundled/group">}}) to create a group called "developers" and the [`user` resource]({{< relref "/client/19/resources/bundled/">}}) to create a user, give them properties, and assign them to the developers group.
...
Use the [`template` resource]({{< relref "/client/19/resources/bundled/template">}}) create the `/etc/app_name/config.yml` file using the `config.yml.erb` template.
...
Some common resource functionality is also supported, as long as the value of a property can be expressed as one of the four primitive types (string, integer, boolean, array). That means it's possible to use [`only_if` or `not_if` guards]({{< relref "/client/19/resources/common_functionality/#guards" >}}) as long as they shell out to Bash or PowerShell and aren't passed a Ruby block.
