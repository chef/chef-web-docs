...
Chef 360 Platform 1.7.3 adds InSpec profile support through the Automate Connector, introduces a backup upload workflow for large file migrations, provides configurable replica scaling for the Automate Connector in on-premises deployments, and resolves bugs affecting login and node enrollment.

### Prerequisites

Before upgrading to Chef 360 Platform 1.7.3, you must be running at least version 1.7.1. For more information, see the [supported upgrade path](https://docs.chef.io/360/1.7/admin_console/update/).

### New features

- **Automate Connector InSpec profile support**: The Automate Connector now proxies Chef InSpec compliance profile requests through Chef Declarative State Management (DSM) to Chef Automate, letting nodes managed through Chef 360 fetch InSpec profiles and run compliance scans end-to-end.

- **`upload-backup` CLI command for large file migrations**: A new `upload-backup` CLI command lets you upload Chef Infra Server backup files larger than 5 GB to Chef 360 Platform.
  Previously, large backup archives couldn't be reliably uploaded through the S3 path; this command removes that limitation for customers migrating large datasets.

- **Automate Connector replica scaling**: You can now configure the minimum number of ingest service pods for on-premises Automate Connector deployments from the Admin Console.
  The default minimum pod count is 1, and the setting includes a description of how changing the pod count affects Automate Connector performance, autoscaling behavior, and CPU utilization.

### Bug fixes

<!-- vale Microsoft.Contractions["does not"] = NO -->

- **WAF rules blocking Automate Connector requests**: Fixed an issue where WAF rules blocked Automate Connector data-collector requests with `CrossSiteScripting_Body` pattern matches, causing 403 Forbidden errors during Chef Infra Client runs forwarded through the connector.

- **Backup upload failures in FIPS environments**: Fixed backup upload failures with "OpenSSL does not support FIPS mode" errors when importing Chef Infra Server backups into Chef 360 clusters operating in FIPS-compliant environments.

- **Chef 360 login page rendering issues**: Fixed an issue where elements on the Chef 360 login page didn't render or respond correctly.

- **Node enrollment in proxy-restricted environments**: Fixed an issue where `chef-node-enrollment-cli` ignored `http_proxy`, `https_proxy`, and `no_proxy` environment variables, causing node enrollment to hang in proxy-restricted environments when `hab-sup` couldn't download required packages.

<!-- vale Microsoft.Contractions["does not"] = YES -->

### Supported skill versions

This release supports the following skill versions.

| Skill | Skill name | Habitat package | Version | Change summary |
| ----- | ---------- | --------------- | ------- | -------------- |
| Courier Runner | courier-runner | chef-platform/courier-runner | 1.5.19 | |
| Gohai | chef-gohai | chef-platform/chef-gohai | 1.2.57 | |
| InSpec Interpreter | inspec-interpreter | chef-platform/inspec-interpreter | 1.0.24 | |
| Node Management Agent | node-management-agent | chef-platform/node-management-agent | 1.1.18 | |
| Restart Interpreter | restart-interpreter | chef-platform/restart-interpreter | 1.0.21 | |
| Shell Interpreter | shell-interpreter | chef-platform/shell-interpreter | 1.0.18 | |
| Infra Client Interpreter | chef-client-interpreter | chef-platform/chef-client-interpreter | 1.0.20 | |
| Ansible Interpreter | ansible-interpreter | chef-platform/ansible-interpreter | 0.1.11 | |

## Chef 360 Platform 1.7.2

Release date: June 23, 2026

Chef 360 Platform 1.7.2 delivers major improvements across Node Management and Courier, introduces a new notifications system, and adds platform capabilities for tenant lifecycle management and security updates.

### Prerequisites

Before upgrading to Chef 360 Platform 1.7.2, you must be running version 1.7.1.

### New features

- **Node Management skill lifecycle hooks**: You can now register scripts with Node Management that run automatically during skill installation, upgrade, and removal events.
  This eliminates manual post-install steps such as bootstrapping Chef Infra Client into Chef Declarative State Management (DSM) and provisioning credentials.

- **Courier: run jobs on demand**: You can now manually execute any existing Courier job definition without creating a duplicate.
  Manual execution creates a new job instance linked to the same job definition, preserving the original schedule and execution history.

- **Notifications system**: Chef 360 Platform now includes a built-in notifications system integrated with Node Management and Courier.
  You can subscribe to platform events---such as node enrollment status changes and Courier job execution failures---to receive timely alerts about platform activity.

- **Automate Connector**: When migrating from Chef Infra Server to Chef Declarative State Management (DSM), you can now configure Chef DSM to forward Chef Infra Client run reports to your existing Chef Automate server.
  This preserves your reporting and compliance workflows during migration without requiring changes to your Chef clients.

### Improvements

- **Courier job wizard: edit advanced fields**: You can now edit advanced Courier job fields---including input variables, expected input variables, conditions, and output variables---from any step in the Courier job creation wizard in the Chef 360 Platform UI before final submission.

- **License management API**: The license management API now includes endpoints to list and get services, consistent with the existing assets and features API conventions.

### Dependency updates

- **Go runtime**: Updated Go runtime to version 1.26.3 on Chef 360 Platform services to resolve Go CVEs.

### Security updates

- Resolved critical and high CVEs in fluent-bit, mailpit, nginx, and chef-os-shell.
- Updated the Go Habitat package to 1.26.3 to resolve Go CVEs.
...
| Skill                    | Skill name              | Habitat package                       | Version | Change summary                                                 |
| ------------------------ | ----------------------- | ------------------------------------- | ------- | -------------------------------------------------------------- |
| Courier Runner           | courier-runner          | chef-platform/courier-runner          | 1.5.19  | Courier runner handles interpreter exit codes gracefully.      |
| Gohai                    | chef-gohai              | chef-platform/chef-gohai              | 1.2.57  | Improvements in skill configuration validation.                |
| InSpec Interpreter       | inspec-interpreter      | chef-platform/inspec-interpreter      | 1.0.24  | System environment variables inherited by InSpec at execution. |
| Node Management Agent    | node-management-agent   | chef-platform/node-management-agent   | 1.1.18  | Improvement in subnet mask detection for node attributes.      |
| Restart Interpreter      | restart-interpreter     | chef-platform/restart-interpreter     | 1.0.21  |                                                                |
| Shell Interpreter        | shell-interpreter       | chef-platform/shell-interpreter       | 1.0.18  |                                                                |
| Infra Client Interpreter | chef-client-interpreter | chef-platform/chef-client-interpreter | 1.0.20  |                                                                |
| Ansible Interpreter      | ansible-interpreter     | chef-platform/ansible-interpreter     | 0.1.11  |                                                                |
...
| ----------------------- | ---------------- |
| chef-360                | 1.7.2            |
| Installer               | v2.14.0+k8s-1.32 |
| Kubernetes (bundled)    | v1.32.12+k0s.1   |
| AdminConsole            | v1.129.4-ec.0    |
| EmbeddedClusterOperator | v2.14.0+k8s-1.32 |
| OpenEBS                 | v4.4.0           |
| Registry                | v3.0.0           |
| SeaweedFS               | v4.0.413         |
| Velero                  | v11.4.0          |

## Chef 360 Platform 1.7.1

Release date: May 12, 2026

Chef 360 Platform 1.7.1 is a patch release focused on security hardening, SSO stability, deployment reliability with custom certificates, and migration bug fixes.

### Prerequisites

You must upgrade to Chef 360 Platform 1.7.0 before upgrading to 1.7.1.

### New features

- Chef 360 Platform disaster recovery now supports S3-compatible MinIO. Organizations operating in air-gapped or on-premises environments benefit from greater flexibility in their disaster recovery strategies.
- Chef's AI Assistant (Beta) is now available for SaaS customers on an invite-only basis.

### Bug fixes

- Fixed multiple SSO issues, including login page redirect failures after logout and intermittent authentication errors.
- Fixed an issue where cookbook contents couldn't be viewed in the UI after migrating from Chef Infra Server to Chef DSM.
- Fixed deployment failures when using custom certificates larger than approximately 100 KB.
- Fixed upgrade failures when upgrading from 1.6.1 to 1.7.0 in HyperscaleHA topology.
...
| Skill                    | Skill name              | Habitat package                       | Version | Change summary                                                     |
| ------------------------ | ----------------------- | ------------------------------------- | ------- | ------------------------------------------------------------------ |
| Courier Runner           | courier-runner          | chef-platform/courier-runner          | 1.5.16  | Courier Runner handles interpreter exit codes gracefully           |
| Gohai                    | chef-gohai              | chef-platform/chef-gohai              | 1.2.54  | Improvements in skill configuration validation                     |
| InSpec Interpreter       | inspec-interpreter      | chef-platform/inspec-interpreter      | 1.0.21  | System environment variables inherited by Chef InSpec at execution |
| Node Management Agent    | node-management-agent   | chef-platform/node-management-agent   | 1.1.14  | Improvement in subnet mask detection for node attributes           |
| Restart Interpreter      | restart-interpreter     | chef-platform/restart-interpreter     | 1.0.18  |                                                                    |
| Shell Interpreter        | shell-interpreter       | chef-platform/shell-interpreter       | 1.0.15  |                                                                    |
| Infra Client Interpreter | chef-client-interpreter | chef-platform/chef-client-interpreter | 1.0.18  |                                                                    |
| Ansible Interpreter      | ansible-interpreter     | chef-platform/ansible-interpreter     | 0.1.10  |                                                                    |
...
## Chef 360 Platform 1.7.0

Release date: April 7, 2026

### Overview

Chef 360 Platform 1.7.0 is a significant step toward enterprise readiness and production deployments, with a strong focus on reliability, security, and operational control.
This release removes key adoption barriers for regulated and large-scale environments by introducing Bring Your Own Kubernetes (BYOK) and built-in disaster recovery (DR).

### New features

#### Bring Your Own Kubernetes (BYOK)

You can now deploy Chef 360 Platform on customer-managed Kubernetes clusters, enabling organizations to run the platform within their own infrastructure and networking boundaries.

Key benefits:

- Full control over the underlying infrastructure environment
- Flexibility to deploy Chef 360 Platform in on-premises, cloud, or air-gapped environments
- Clear installation paths with pre-install checks and documented prerequisites

#### Disaster recovery (DR)

Chef 360 Platform now includes built-in disaster recovery, eliminating the need for manual Kubernetes or YAML-based backup workflows.

Key capabilities:

- Dedicated disaster recovery section in the Admin Console
- UI-based configuration for:
  - Backup and snapshot schedules
  - Retention policies
  - External storage locations
- Visibility into backup health, including:
  - Recent runs
  - Success and failure status

### Bug fixes

You can now create organization names with hyphens (`-`).

### Skills

#### Skill updates

- Courier Runner (1.5.14): Courier runner handles interpreter exit codes gracefully.
- Gohai (1.2.53): Improvements in skill configuration validation.
- InSpec Interpreter (1.0.20): System environment variables inherited by Chef InSpec at execution.
- Node Management Agent (1.1.13): Improvement in subnet mask detection for node attributes.
...
| ------------------------ | ----------------------- | ------------------------------------- | ------- |
| Courier Runner           | courier-runner          | chef-platform/courier-runner          | 1.5.14  |
| Gohai                    | chef-gohai              | chef-platform/chef-gohai              | 1.2.53  |
| InSpec Interpreter       | inspec-interpreter      | chef-platform/inspec-interpreter      | 1.0.20  |
| Node Management Agent    | node-management-agent   | chef-platform/node-management-agent   | 1.1.13  |
| Restart Interpreter      | restart-interpreter     | chef-platform/restart-interpreter     | 1.0.15  |
| Shell Interpreter        | shell-interpreter       | chef-platform/shell-interpreter       | 1.0.14  |
| Infra Client Interpreter | chef-client-interpreter | chef-platform/chef-client-interpreter | 1.0.15  |
| Ansible Interpreter      | ansible-interpreter     | chef-platform/ansible-interpreter     | 0.1.9   |

## Chef 360 Platform 1.6.1

Release date: March 17, 2026

Chef 360 Platform 1.6.1 adds node archival, deeper DSM integration, security upgrades, and performance and stability improvements across the platform.

### New features

- Added Chef Declarative State Management (DSM) integration and migration improvements, including new import services, workers, and CLIs. For DSM, knife commands for organizations, users, and ACLs are now replaced by Chef 360 Platform CLI commands.
- Added support for migrating from Chef Infra Server to Chef DSM with guided migration steps and minimal operational disruption. For more information, see [Chef Infra Server migration](https://docs.chef.io/360/1.6/dsm/chef_server_migration/).
- Added node archival capabilities for Chef 360 Platform to keep node lists clean and exclude stale nodes from operations.

### Improvements

- Added Policyfiles and policy groups to the Chef 360 Platform UI.
- Added a JSON code editor to the Courier job creation wizard.
- Made UX improvements on DSM pages.

### Bug fixes

- Fixed an issue where excessive RDS pod memory caused crashes in other pods.

### Supported skills

| Skill                    | Skill name              | Habitat package                       | Version | Change summary                                                |
| ------------------------ | ----------------------- | ------------------------------------- | ------- | ------------------------------------------------------------- |
| Courier Runner           | courier-runner          | chef-platform/courier-runner          | 1.5.12  | Courier runner handles interpreter exit codes gracefully      |
| Gohai                    | chef-gohai              | chef-platform/chef-gohai              | 1.2.49  | Improvements in skill configuration validation                |
| InSpec Interpreter       | inspec-interpreter      | chef-platform/inspec-interpreter      | 1.0.18  | System environment variables inherited by InSpec at execution |
| Node Management Agent    | node-management-agent   | chef-platform/node-management-agent   | 1.1.11  | Improvement in subnet mask detection for node attributes      |
| Restart Interpreter      | restart-interpreter     | chef-platform/restart-interpreter     | 1.0.13  |                                                               |
| Shell Interpreter        | shell-interpreter       | chef-platform/shell-interpreter       | 1.0.12  |                                                               |
| Infra Client Interpreter | chef-client-interpreter | chef-platform/chef-client-interpreter | 1.0.12  |                                                               |
| Ansible Interpreter      | ansible-interpreter     | chef-platform/ansible-interpreter     | 0.1.8   |                                                               |
...
- **DSM in the Chef 360 Platform User Interface (UI):** You can view and manage DSM nodes and related objects (such as cookbooks, roles, databags, environments, and clients) directly from the Chef 360 Platform web interface, without relying on API or CLI access.
...
- **Multi-node High Availability (HA) support:** Chef 360 Platform now supports configuring and managing multi-node HA deployments, enabling resilient cluster topologies for improved availability and fault tolerance.
...
- The Chef 360 Platform graphical user interface now supports language localization.
...
- Chef 360 Platform SMTP settings now supports configurations for SendGrid to send emails.
