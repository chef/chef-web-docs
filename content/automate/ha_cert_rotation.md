...
[Existing certificates](/automate/ha_cert_self_sign/) can be used to generate new ones.
...
{{< warning >}}
It's critical to generate the certificate template using the following command:
`chef-automate cert-rotate generate-certificate-config certificate-config.toml`
