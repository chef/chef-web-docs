...
For more information on Chef licenses, see [Chef licensing documentation](/licensing/).
...
Once a license expires, you can't use Chef Automate features and capabilities. All trial licenses will cease to operate once the expiry date has elapsed. To continue using Chef Automate, you must upgrade to an Enterprise License.
...
You can't use Chef Automate features and capabilities. To continue using Chef Automate, apply again for the Chef Automate license using the following steps:

1. [Contact us](https://www.chef.io/contact-us) to get a license.
1. If you already have a license key, paste it in the popup box and check the I agree to the Terms and Service box.
1. Select **Apply License**.

## Chef Infra Server under Automate License

Chef Infra Server, when deployed with Automat,e will abide by the Automate license.

The `chef-server-ctl` command won't work if the commercial/trial license has not been applied to Automate or has expired. In the case of `knife` or `Infra Client` execution, the Chef Infra Server won't respond if the Automate license has not been applied or expired.
