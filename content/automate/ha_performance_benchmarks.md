...
For PostgreSQL, we prefer vertical scaling to horizontal scaling for high-availability deployments. All frontend nodes in this architecture directly talk to the leader node, hence it's the sole point of interaction. Increasing the number of follower nodes doesn't share the load but adds to the load on the leader since it has to replicate data to all followers. It adds extra replication lag and worsens performance overall.
