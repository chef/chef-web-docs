...
Now that `knife` is configured, copy the TLS/SSL certificates from your Chef
...
2.  If an TLS/SSL notice is shown due to your self-signed certificate while
    connecting to Chef Supermarket via a web browser, accept the TLS/SSL
    certificate. A trusted TLS/SSL certificate should be used for private
