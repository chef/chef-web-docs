...
[menu]
  [menu.inspec]
    title = "Chef InSpec Reporters"
    identifier = "inspec/reference/reporters.md Chef InSpec Reporters"
...
Configure the reporter(s) using either the `--reporter` option or as part of the
general config file using the `--config` (or `--json-config`, prior to v3.6)
option. While you can configure multiple reporters to write to different files,
only one reporter can output to the screen(stdout).
