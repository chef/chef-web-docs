...
:   Verify the TLS/SSL certificate on the Chef Infra Server. When `true`,
    Chef Infra Client always verifies the TLS/SSL certificate. When `false`,
    Chef Infra Client uses the value of `ssl_verify_mode` to determine
    if the TLS/SSL certificate requires verification.
...
:   The TLS/SSL certificate of your Chef Infra Server that will be added to
...
**Installing the Azure Chef extension on a Linux system with TLS/SSL peer
