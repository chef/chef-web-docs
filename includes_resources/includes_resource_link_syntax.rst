...
The full syntax for all of the properties that are available to the |resource link| resource is:
...
* ``group``, ``link_type``, ``mode``, ``owner``, ``provider``, ``target_file``, and ``to`` are properties of this resource, with the |ruby| type shown. |see attributes|
