...
* ``destination``, ``group``, ``provider``, ``repository``, ``revision``, ``svn_arguments``, ``svn_info_args``, ``svn_password``, ``svn_username``, and ``user`` are properties of this resource, with the |ruby| type shown. |see attributes|
