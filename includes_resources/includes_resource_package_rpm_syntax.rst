...
The full syntax for all of the properties that are available to the |resource package_rpm| resource is:
...
* ``allow_downgrade``, ``options``, ``package_name``, ``provider``, ``source``, ``timeout``, and ``version`` are properties of this resource, with the |ruby| type shown. |see attributes|
