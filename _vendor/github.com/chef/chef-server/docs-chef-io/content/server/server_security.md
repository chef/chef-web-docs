...
Chef Infra Server's TLS/SSL certificate.

### Automatic Installation (recommended)

The Chef Infra Server can be configured to use TLS/SSL certificates by
...
<td>The TLS/SSL certificate used to verify communication over HTTPS.</td>
</tr>
<tr class="even">
<td><code>nginx['ssl_certificate_key']</code></td>
<td>The certificate key used for TLS/SSL communication.</td>
</tr>
</tbody>
</table>
...
**Example: Configure TLS/SSL Keys for Nginx**

The following example shows how the Chef Infra Server sets up and
configures TLS/SSL certificates for Nginx. The cipher suite used by Nginx
...
information on how knife and Chef Infra Client use TLS/SSL certificates
...
To regenerate TLS/SSL certificates:
...
3.  If your organization has provided custom TLS/SSL certificates to the
...
4.  Run the following command, Chef server-generated TLS/SSL certificates
...
## TLS/SSL Encryption Between Chef Infra Server and External PostgreSQL
...
3.  Ensure that TLS/SSL support is compiled in on PostgreSQL. This applies
    whether you are compiling your own source or using a pre-compiled
    binary.

4.  Place TLS/SSL certificates in the proper directories on the PostgreSQL
    machine and ensure they have correct filenames, ownerships, and
    permissions.

5.  Enable TLS/SSL on PostgreSQL by editing the `postgresql.conf` file. Set
    `ssl = on` and specify the paths to the TLS/SSL certificates:
...
10. Verify that TLS/SSL is enabled and that TLS/SSL connections are up between
    ...
    true (`t`) in rows with TLS/SSL connections:
