...
- An TLS/SSL certificate on the target node
...
WinRM TLS/SSL listeners. This argument also generates a related public key
...
: The password for the TLS/SSL certificate.
...
certificate store so that it may be used as the TLS/SSL certificate by a
WinRM listener.
...
: The thumbprint of the TLS/SSL certificate. Required when the `--cert-install` option is not part of a command.
...
: Optional. The certificate authority (CA) trust file used for TLS/SSL transport.
...
: TLS/SSL Cert Fingerprint to bypass normal cert chain checks
...
**Generate an TLS/SSL certificate, and then create a listener**
...
Next, create the TLS/SSL listener:
...
The TLS/SSL listener is created and should be listening on TCP port `5986`,
which is the default WinRM TLS/SSL port.
