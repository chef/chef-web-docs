...
#. Login using |ssh| to access the |chef compliance| instance and update the software. Use the |ssh| key pair and the IP address or public hostname that was automatically assigned when the |amazon ami| was launched. The default user is ``ec2-user``. For example:
